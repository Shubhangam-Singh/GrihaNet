  ...
  return h("div", {
    ...
    h("div", { style: { padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: "1px solid var(--border)" } },
      ...
      h("div", { style: { display: "flex", gap: 8, alignItems: "center" } },
        ...
        onDelete && h("button", {
          "data-tooltip": "Remove this camera", onClick: () => onDelete(cam),
          style: {
...
function AutomationModal({ appliances, onClose, onCreate }) {
  ...
  const sel = (val, onChange, opts) => h("select", {
  ...
  const inp2 = (val, onChange, ph = "", type = "text") => h("input", {
    type, placeholder: ph, value: val,
  ...
  return h("div", { style: overlay, onClick: onClose },
    h("div", {
      ...
      style: {
        width: 480, borderRadius: 16, background: "var(--bg-card)", border: "1px solid var(--border)",
        padding: 28, boxShadow: "0 24px 60px rgba(0,0,0,.5)"
      }
    },
      h("div", { style: { display: "flex", gap: 8, marginBottom: 20 } },
        stepLabel.map((l, i) => h("div", {
          key: i, style: {
            flex: 1, textAlign: "center", padding: "6px 0",
            borderRadius: 8, fontSize: 11, fontWeight: 700,
            background: step === i + 1 ? T.gradient1 : step > i + 1 ? T.accentDim : T.surface,
            color: step === i + 1 ? "#000" : step > i + 1 ? T.accent : T.textMuted, border: "1px solid var(--border)"
          }
        }, l))
      ),
      ...
      h("div", { style: { display: "flex", justifyContent: "space-between", marginTop: 20 } },
        ...
        step < 3 ? h("button", {
          ...
          h("button", {
            onClick: handleCreate, disabled: saving || !ruleName.trim(),
            style: {
              padding: "10px 20px", borderRadius: 10, border: "none",
                ...
                u.is_superadmin
                  ? h("span", {
                    style: {
                      ...
                      color: "#f59e0b", fontSize: 10, fontWeight: 700, letterSpacing: .8
                    }
                  }, "👑 SUPER ADMIN")
              ...
              h("td", { style: { padding: "12px 0", textAlign: "right" } },
                u.is_superadmin
                  ...
                  ? h("div", { style: { display: "flex", gap: 8, justifyContent: "flex-end", alignItems: "center" } },
                    ...
                    h("button", {
                      onClick: () => resetPassword(u), title: "Reset Password",
                      style: {
                    ...
                    h("button", {
                      disabled: true, title: "Cannot promote/demote Super Admin",
                      style: {
                        background: "transparent", border: `1px solid ${T.border}33`, color: "var(--text-muted)",
                        padding: "6px 10px", borderRadius: 6, cursor: "not-allowed", fontSize: 13, opacity: .3
                      }
                    }, "🛡️"),
                    h("button", {
                      disabled: true, title: "Cannot suspend Super Admin",
                      style: {
                        background: "transparent", border: `1px solid ${T.border}33`, color: "var(--text-muted)",
                        padding: "6px 10px", borderRadius: 6, cursor: "not-allowed", fontSize: 13, opacity: .3
                      }
                    ...
                    h("button", {
                      disabled: true, title: "Cannot delete Super Admin",
                      style: {
                        background: "transparent", border: `1px solid ${T.border}33`, color: "var(--text-muted)",
                        padding: "6px 10px", borderRadius: 6, cursor: "not-allowed", fontSize: 13, opacity: .3
                      }
                  ...
                  : h("div", { style: { display: "flex", gap: 8, justifyContent: "flex-end" } },
                    h("button", {
                      onClick: () => toggleRole(u), title: "Promote/Demote Role",
                      style: {
                        background: "transparent", border: "1px solid var(--border)", color: "var(--text)",
                        padding: "6px 10px", borderRadius: 6, cursor: "pointer", fontSize: 13
                      }
                    ...
                    h("button", {
                      onClick: () => deactivate(u), title: u.is_active ? "Suspend user" : "Activate user",
                      style: {
                      ...
                      u.is_active ? "⏸" : "▶"),
                    h("button", {
                      onClick: () => deleteUser(u), title: "Delete entirely",
                      style: {
                        background: "transparent", border: `1px solid ${T.red}44`, color: "var(--danger)",
                        padding: "6px 10px", borderRadius: 6, cursor: "pointer", fontSize: 13
                      }
                    }, "🗑️")
                  )
  ...
  const processCommand = useCallback((raw) => {
    ...
    if (txt.includes("good morning")) {
      ...
      appliances.forEach(a => {
        if (a.name.toLowerCase().includes("geyser") || a.name.toLowerCase().includes("kitchen") || a.name.toLowerCase().includes("bedroom light")) { if (!a.on) { toggleAppliance(a.id); count++; } }
        else if (a.name.toLowerCase().includes("outside")) { if (a.on) toggleAppliance(a.id); }
      });
      showFeedback(true, "Good morning! Preparing home."); return;
    }
    if (txt.includes("good night") || txt.includes("sleep")) {
      appliances.forEach(a => {
        if (!a.name.toLowerCase().includes("router") && !a.name.toLowerCase().includes("fridge")) { if (a.on) toggleAppliance(a.id); }
      });
      showFeedback(true, "Good night! Sleeping mode activated."); return;
    }
    if (txt.includes("eco mode") || txt.includes("power save")) {
      appliances.forEach(a => {
        if (a.name.toLowerCase().includes("tv") || a.name.toLowerCase().includes("geyser") || a.name.toLowerCase().includes("ac")) { if (a.on) toggleAppliance(a.id); }
      });
      showFeedback(true, "Eco mode activated. Heavy appliances off."); return;
    }
    ...
    showFeedback(false, `"${txt}" — try: "turn on geyser" or "eco mode"`);
...
function GrihaNet() {
  ...
  const [meterData, setMeterData] = useState({ vrms: 0, irms: 0, pf: 0, S: 0, P: 0, Q: 0, vthd: 0, ithd: 0, vpeak: 0, ipeak: 0 });
  ...
  /* ─── Live Meter Polling (every 3 s when on power tab) ─── */
  useEffect(() => {
    if (!loggedIn) return;
    const poll = async () => {
      const res = await api.get("/power/meter");
      if (res && typeof res.vrms !== "undefined") setMeterData(res);
    };
    poll();
  ...
  /* ─── Live Alerts Polling (AI Notifications) ─── */
  useEffect(() => {
    if (!loggedIn || !settings.pushNotifications) return;
    const pollAlerts = async () => {
      const res = await api.get("/alerts/");
      // The API returns { alerts: [...] } where each alert has 'id', 'icon', 'type', 'msg', 'read'.
      if (res && res.alerts) {
        setAlerts(prev => {
          ...
          newAlerts.forEach(a => {
            // Only pop toast if it's high severity or AI detection
            addToast(a.icon || "🚨", "Security Alert", a.msg, a.type === "danger" ? T.red : T.orange);
          });
          return res.alerts;
        });
      }
    };
    const i = setInterval(pollAlerts, 5000);
    return () => clearInterval(i);
  }, [loggedIn, settings.pushNotifications, addToast]);
  ...
  return h("div", { style: { minHeight: "100vh", background: "transparent", color: "var(--text)", fontFamily: "'DM Sans',sans-serif" } },
    ...
    h("header", {
      ...
      h("div", { style: { display: "flex", alignItems: "center", gap: 12 } },
        ...
        /* Mic button */
        h("button", {
          onClick: startListening, title: listening ? "Stop" : "Voice command", style: {
        ...
        /* Refresh */
        h("button", {
          onClick: fetchAll, title: "Refresh", style: {
        ...
        /* Bell */
        h("button", {
          onClick: () => setTab("alerts"), title: "Alerts", style: {
        ...
        /* User avatar */
        h("div", {
          onClick: doLogout, title: `Logout (${user?.name || "user"})`, style: {
    ...
    (listening || feedback) && h("div", {
      style: {
        position: "fixed", bottom: 24, left: "50%", transform: "translateX(-50%)",
        background: "var(--bg-card)", border: `1px solid ${listening ? T.accent : feedback?.ok ? T.accent : T.orange}`,
        borderRadius: 16, padding: "14px 24px", zIndex: 5000, boxShadow: "0 8px 32px rgba(0,0,0,.4)",
      ...
      h("div", null,
        ...
        listening && h("div", { style: { fontSize: 11, color: "var(--text-muted)", marginTop: 2 } },
          'Try: "good night" • "eco mode" • "turn on lights"')
      )
    ),
    ...
    h("main", { style: { padding: "20px 24px 40px", maxWidth: 1280, margin: "0 auto", paddingTop: "128px" } },
      ...
      tab === "power" && h(React.Fragment, null,
        ...
        h("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 12, marginBottom: 16 } },
          ...
          h("div", { className: "stat-card fadeUp d2", "data-tooltip": "Live RMS Current from meter" },
            ...
            h("div", { style: { position: "relative" } },
              ...
              h("div", { style: { display: "flex", alignItems: "baseline", gap: 2, marginBottom: 6 } },
                h("span", { className: "live-dot", style: { marginBottom: 1, marginRight: 4 } }),
                h("span", { style: { fontSize: 28, fontWeight: 700, fontFamily: "'IBM Plex Mono'", color: "var(--teal)", lineHeight: 1 } }, meterData.irms > 0 ? meterData.irms.toFixed(2) : (liveWatts / 230).toFixed(2)),
                h("span", { style: { fontSize: 12, color: "var(--text-muted)", marginLeft: 5 } }, "A")
              ),
              h("span", { style: { fontSize: 11, color: "var(--text-dim)" } }, meterData.irms > 0 ? `Peak: ${meterData.ipeak.toFixed(2)}A · PF: ${meterData.pf.toFixed(2)}` : "Calculated Total")
            )
          ),
          h("div", { className: "stat-card fadeUp d3", "data-tooltip": "Live RMS Grid Voltage from meter" },
            ...
            h("div", { style: { position: "relative" } },
              ...
              h("div", { style: { display: "flex", alignItems: "baseline", gap: 2, marginBottom: 6 } },
                h("span", { className: "live-dot", style: { marginBottom: 1, marginRight: 4 } }),
                h("span", { style: { fontSize: 28, fontWeight: 700, fontFamily: "'IBM Plex Mono'", color: "var(--blue)", lineHeight: 1 } }, meterData.vrms > 0 ? meterData.vrms.toFixed(1) : "230"),
                h("span", { style: { fontSize: 12, color: "var(--text-muted)", marginLeft: 5 } }, "V")
              ),
              h("span", { style: { fontSize: 11, color: "var(--text-dim)" } }, meterData.vrms > 0 ? `Peak: ${meterData.vpeak.toFixed(1)}V` : "Standard Supply")
            )
          ),
        ...
        /* ── Live Meter Panel ── */
        h(Card, { className: "fadeUp d6", style: { marginBottom: 14 } },
          h("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 } },
            h("div", null,
              h("div", { style: { fontSize: 14, fontWeight: 700, color: "var(--text)" } }, "⚡ Live Meter Data"),
              h("div", { style: { fontSize: 11, color: "var(--text-dim)", marginTop: 2 } }, "Real-time power quality readings from hardware sensor")
            ),
            h("div", { style: { display: "flex", alignItems: "center", gap: 8 } },
              (meterData.vrms > 0 || meterData.irms > 0 || meterData.pf !== 0 || meterData.P !== 0) ? h("span", { className: "badge badge-success" }, "● Live") : h("span", { className: "badge badge-warn" }, "○ No Signal")
            )
          ),
          h("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 10 } },
            /* Voltage RMS */
            h("div", { style: { background: "var(--bg-input)", borderRadius: 10, padding: "12px 14px", border: "1px solid var(--border)" } },
              h("div", { style: { fontSize: 10, color: "var(--text-dim)", fontWeight: 600, letterSpacing: "0.05em", textTransform: "uppercase", marginBottom: 6 } }, "Voltage RMS"),
              h("div", { style: { display: "flex", alignItems: "baseline", gap: 4 } },
                h("span", { style: { fontSize: 22, fontWeight: 700, fontFamily: "'IBM Plex Mono'", color: "var(--blue)" } }, meterData.vrms.toFixed(1)),
                h("span", { style: { fontSize: 11, color: "var(--text-muted)" } }, "V")
              ),
              h("div", { style: { fontSize: 10, color: "var(--text-dim)", marginTop: 4 } }, `Peak: ${meterData.vpeak.toFixed(1)} V`)
            ),
            /* Current RMS */
            h("div", { style: { background: "var(--bg-input)", borderRadius: 10, padding: "12px 14px", border: "1px solid var(--border)" } },
              h("div", { style: { fontSize: 10, color: "var(--text-dim)", fontWeight: 600, letterSpacing: "0.05em", textTransform: "uppercase", marginBottom: 6 } }, "Current RMS"),
              h("div", { style: { display: "flex", alignItems: "baseline", gap: 4 } },
                h("span", { style: { fontSize: 22, fontWeight: 700, fontFamily: "'IBM Plex Mono'", color: "var(--teal)" } }, meterData.irms.toFixed(2)),
                h("span", { style: { fontSize: 11, color: "var(--text-muted)" } }, "A")
              ),
              h("div", { style: { fontSize: 10, color: "var(--text-dim)", marginTop: 4 } }, `Peak: ${meterData.ipeak.toFixed(2)} A`)
            ),
            /* Active Power */
            h("div", { style: { background: "var(--bg-input)", borderRadius: 10, padding: "12px 14px", border: "1px solid var(--border)" } },
              h("div", { style: { fontSize: 10, color: "var(--text-dim)", fontWeight: 600, letterSpacing: "0.05em", textTransform: "uppercase", marginBottom: 6 } }, "Active Power (P)"),
              h("div", { style: { display: "flex", alignItems: "baseline", gap: 4 } },
                h("span", { style: { fontSize: 22, fontWeight: 700, fontFamily: "'IBM Plex Mono'", color: "var(--warn)" } }, meterData.P.toFixed(1)),
                h("span", { style: { fontSize: 11, color: "var(--text-muted)" } }, "W")
              ),
              h("div", { style: { fontSize: 10, color: "var(--text-dim)", marginTop: 4 } }, "Real power consumed")
            ),
            /* Apparent Power */
            h("div", { style: { background: "var(--bg-input)", borderRadius: 10, padding: "12px 14px", border: "1px solid var(--border)" } },
              h("div", { style: { fontSize: 10, color: "var(--text-dim)", fontWeight: 600, letterSpacing: "0.05em", textTransform: "uppercase", marginBottom: 6 } }, "Apparent Power (S)"),
              h("div", { style: { display: "flex", alignItems: "baseline", gap: 4 } },
                h("span", { style: { fontSize: 22, fontWeight: 700, fontFamily: "'IBM Plex Mono'", color: "var(--purple)" } }, meterData.S.toFixed(1)),
                h("span", { style: { fontSize: 11, color: "var(--text-muted)" } }, "VA")
              ),
              h("div", { style: { fontSize: 10, color: "var(--text-dim)", marginTop: 4 } }, "Total power drawn")
            ),
            /* Reactive Power */
            h("div", { style: { background: "var(--bg-input)", borderRadius: 10, padding: "12px 14px", border: "1px solid var(--border)" } },
              h("div", { style: { fontSize: 10, color: "var(--text-dim)", fontWeight: 600, letterSpacing: "0.05em", textTransform: "uppercase", marginBottom: 6 } }, "Reactive Power (Q)"),
              h("div", { style: { display: "flex", alignItems: "baseline", gap: 4 } },
                h("span", { style: { fontSize: 22, fontWeight: 700, fontFamily: "'IBM Plex Mono'", color: "var(--blue)" } }, meterData.Q.toFixed(1)),
                h("span", { style: { fontSize: 11, color: "var(--text-muted)" } }, "VAR")
              ),
              h("div", { style: { fontSize: 10, color: "var(--text-dim)", marginTop: 4 } }, "Non-working power")
            ),
            /* Power Factor */
            h("div", { style: { background: "var(--bg-input)", borderRadius: 10, padding: "12px 14px", border: "1px solid var(--border)" } },
              h("div", { style: { fontSize: 10, color: "var(--text-dim)", fontWeight: 600, letterSpacing: "0.05em", textTransform: "uppercase", marginBottom: 6 } }, "Power Factor"),
              h("div", { style: { display: "flex", alignItems: "baseline", gap: 4 } },
                h("span", { style: { fontSize: 22, fontWeight: 700, fontFamily: "'IBM Plex Mono'", color: meterData.pf >= 0.9 ? "var(--success)" : meterData.pf >= 0.7 ? "var(--warn)" : "var(--danger)" } }, meterData.pf.toFixed(2))
              ),
              h("div", { style: { fontSize: 10, color: "var(--text-dim)", marginTop: 4 } }, meterData.pf >= 0.9 ? "✅ Good" : meterData.pf >= 0.7 ? "⚠️ Moderate" : meterData.pf > 0 ? "❌ Poor" : "—")
            ),
            /* Voltage THD */
            h("div", { style: { background: "var(--bg-input)", borderRadius: 10, padding: "12px 14px", border: "1px solid var(--border)" } },
              h("div", { style: { fontSize: 10, color: "var(--text-dim)", fontWeight: 600, letterSpacing: "0.05em", textTransform: "uppercase", marginBottom: 6 } }, "Voltage THD"),
              h("div", { style: { display: "flex", alignItems: "baseline", gap: 4 } },
                h("span", { style: { fontSize: 22, fontWeight: 700, fontFamily: "'IBM Plex Mono'", color: meterData.vthd < 5 ? "var(--success)" : meterData.vthd < 8 ? "var(--warn)" : "var(--danger)" } }, meterData.vthd.toFixed(1)),
                h("span", { style: { fontSize: 11, color: "var(--text-muted)" } }, "%")
              ),
              h("div", { style: { fontSize: 10, color: "var(--text-dim)", marginTop: 4 } }, "Total Harmonic Distortion")
            ),
            /* Current THD */
            h("div", { style: { background: "var(--bg-input)", borderRadius: 10, padding: "12px 14px", border: "1px solid var(--border)" } },
              h("div", { style: { fontSize: 10, color: "var(--text-dim)", fontWeight: 600, letterSpacing: "0.05em", textTransform: "uppercase", marginBottom: 6 } }, "Current THD"),
              h("div", { style: { display: "flex", alignItems: "baseline", gap: 4 } },
                h("span", { style: { fontSize: 22, fontWeight: 700, fontFamily: "'IBM Plex Mono'", color: meterData.ithd < 5 ? "var(--success)" : meterData.ithd < 8 ? "var(--warn)" : "var(--danger)" } }, meterData.ithd.toFixed(1)),
                h("span", { style: { fontSize: 11, color: "var(--text-muted)" } }, "%")
              ),
              h("div", { style: { fontSize: 10, color: "var(--text-dim)", marginTop: 4 } }, "Total Harmonic Distortion")
            )
          ...
          h(Card, { className: "fadeUp d3" },
            ...
            h(ResponsiveContainer, { width: "100%", height: 220 },
              h(BarChart, { data: weeklyData, barGap: 4 },
                ...
                h(Tooltip, {
                  contentStyle: { background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 10, fontSize: 12, color: "var(--text)", padding: "10px 14px" },
        ...
        h(Card, { id: "tour-power-grid", className: "fadeUp d5" },
          h("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 } },
            ...
            h("button", {
              "data-tooltip": "Add a new appliance", onClick: () => setShowAddAppliance(true),
          ...
          h("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(260px,1fr))", gap: 12 } },
            appliances.map(a => h(Card, {
              key: a.id, className: a.on ? "anim-glow-border" : "",
              ...
              h("div", { style: { display: "flex", alignItems: "flex-start", justifyContent: "space-between" } },
                ...
                h("button", {
                  onClick: e => { e.stopPropagation(); deleteAppliance(a.id); }, title: "Remove", style: {
      ...
      tab === "network" && h(React.Fragment, null,
        ...
        h(Card, { id: "tour-network-list", className: "fadeUp d4" },
          ...
          h("div", { style: { position: "relative", marginBottom: 14 } },
            ...
            h("input", {
              type: "text", placeholder: "Search by name, IP, or MAC\u2026", className: "inp", value: deviceSearch,
            ...
            return h("div", {
              ...
              h("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center" } },
                ...
                h("div", { style: { display: "flex", alignItems: "center", gap: 8, flexShrink: 0 } },
                  ...
                  h("button", {
                    "data-tooltip": d.blocked ? "Allow internet access" : "Block from internet",
      ...
      tab === "cameras" && h(React.Fragment, null,
          ...
          h("div", { style: { background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 24, padding: 32, width: "100%", maxWidth: 420, boxShadow: "0 20px 60px rgba(0,0,0,.6)", animation: "zoomIn .3s ease" } },
            ...
            h("form", { onSubmit: addCamera, style: { display: "flex", flexDirection: "column", gap: 14 } },
              h("div", null,
                ...
                h("input", {
                  placeholder: "e.g. Front Door, Backyard", required: true, value: camForm.name,
              ...
              h("div", null,
                ...
                h("input", {
                  placeholder: "e.g. Ground Floor, Gate", required: true, value: camForm.location,
              ...
              h("div", null,
                ...
                h("input", {
                  placeholder: "rtsp:// or http:// stream address", value: camForm.stream_url,
              ...
              h("div", { style: { display: "flex", gap: 12, marginTop: 6 } },
                h("button", {
                  type: "button", onClick: () => setShowAddCamera(false),
                  style: {
                    flex: 1, padding: "11px", borderRadius: 10, border: "1px solid var(--border)",
                    background: "transparent", color: "var(--text)", cursor: "pointer", fontWeight: 600, fontFamily: "inherit"
                  }
                }, "Cancel"),
                h("button", {
                  type: "submit",
                  style: {
                    ...
                    background: T.accent, color: "#111", cursor: "pointer", fontWeight: 700, fontFamily: "inherit"
                  }
                }, "Add Camera")
              )
            )
        ...
        h("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 } },
          ...
          h("button", {
            "data-tooltip": "Add a new camera", onClick: () => setShowAddCamera(true),
            style: {
              padding: "7px 16px", borderRadius: 8, border: "none", background: "var(--teal-glow)",
          ...
          cameras.length === 0
            ? h(Card, { style: { textAlign: "center", padding: 40, color: "var(--text-muted)" } },
              ...
              h("button", {
                ...
                style: {
                  padding: "10px 24px", borderRadius: 10, border: "none", background: T.accent,
                  color: "#111", fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans'"
                }
              }, "+ Add Camera")
            )
        ...
        h(Card, { className: "fadeUp d5" },
          ...
          h("div", { style: { position: "relative", paddingLeft: 28 } },
            ...
            motionLog.slice(0, 12).map((m, i) => h("div", {
              key: m.id, style: {
      ...
      tab === "alerts" && h(React.Fragment, null,
        ...
        h("div", { style: { display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" } },
          [
            ...
            h("button", {
              key: f.id, onClick: () => setAlertFilter(f.id), style: {
          ...
          (() => {
            ...
            return h("div", { style: { display: "flex", flexDirection: "column", gap: 6 } },
              fa.map(a => {
                ...
                return h("div", {
                  key: a.id, className: "alert-row", style: {
                  ...
                  !a.read && h("button", {
                    "data-tooltip": "Dismiss this alert", onClick: () => dismissAlert(a.id),
                    style: {
      ...
      tab === "automations" && h(React.Fragment, null,
        ...
        h("div", { id: "tour-automations", style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 } },
          ...
          h("button", {
            ...
            style: {
              padding: "10px 20px", borderRadius: 10, border: "none", background: T.gradient1,
