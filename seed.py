...
def seed_database():
    ...
    if Appliance.query.count() == 0:
        ...
        for name, icon, watts, room, is_on in appliances:
            db.session.add(Appliance(
                name=name, icon=icon, watts=watts, room=room, is_on=is_on, user_id=1
            ))
        db.session.commit()
    ...
    if NetworkDevice.query.count() == 0:
        devices = [
            ("Arishem's iPhone", "192.168.1.4", "A4:B1:C2:3D:E5:F6", "phone", 2.4, True, True, False),
            ("Dad's Laptop", "192.168.1.7", "F6:G7:H8:I9:J0:K1", "laptop", 5.8, True, True, False),
            ("Smart TV", "192.168.1.10", "K1:L2:M3:N4:O5:P6", "tv", 14.2, True, True, False),
            ("Mom's Phone", "192.168.1.12", "P6:Q7:R8:S9:T0:U1", "phone", 1.3, True, True, False),
            ("PS5", "192.168.1.15", "U1:V2:W3:X4:Y5:Z6", "gaming", 0.0, False, True, False),
            ("Unknown Device", "192.168.1.22", "Z6:A7:B8:C9:D0:E1", "unknown", 0.8, True, False, False),
        ]
        for name, ip, mac, dtype, bw, online, wl, blocked in devices:
            db.session.add(NetworkDevice(
                name=name, ip=ip, mac=mac, device_type=dtype,
                bandwidth_used=bw, is_online=online,
                is_whitelisted=wl, is_blocked=blocked, user_id=1,
            ))
        db.session.commit()
    ...
    if Camera.query.count() == 0:
        ...
        for name, loc, status in cams:
            db.session.add(Camera(name=name, location=loc, status=status, user_id=1))
        db.session.commit()
    ...
    if Alert.query.count() == 0:
        ...
        for atype, msg, icon, module, read, created in alerts:
            db.session.add(Alert(
                ...
                module=module, is_read=read, user_id=1, created_at=created,
            ))
        db.session.commit()
    ...
    if Settings.query.count() == 0:
        ...
        for k, v in defaults.items():
            db.session.add(Settings(user_id=1, key=k, value=v))
        db.session.commit()
