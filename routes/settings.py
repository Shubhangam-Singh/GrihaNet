...
def generate_pdf_report():
    ...
    try:
        user_id = int(get_jwt_identity())  # JWT identity is always a string — cast to int
        ...
        if not on_appliances:
            pdf.cell(0, 8, "No appliances currently active.", border=1, align="C", ln=True)
        else:
            for idx, a in enumerate(on_appliances):
                fill = idx % 2 != 0
                if fill:
                    pdf.set_fill_color(240, 245, 250)
                pdf.cell(90, 8, a.name, border=1, fill=fill)
                pdf.cell(40, 8, a.room, border=1, fill=fill)
                pdf.cell(60, 8, f"{a.watts} W", border=1, fill=fill, ln=True)
